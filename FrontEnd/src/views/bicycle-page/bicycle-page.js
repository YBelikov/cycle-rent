...
window.addEventListener('load', function () {
  bicycleId = document.getElementById("bicycle-info").getAttribute(
      "data-bicycle-id");
    ...
    $("input[class=custom-control-input]").each((i, item) => {
      ...
      $(item).on("change", () => {
        ...
        if ($(item).prop("checked")) {
          optionPrice[name] = Number(
              $(item).attr("data-price"));
        } else {
          delete optionPrice[name];
        }
...
$("#addToBasket").click(() => {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      $(window.opera ? 'html' : 'html, body').animate({scrollTop: pageTop},
          1000);
    }
  });
})
